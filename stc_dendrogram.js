...
define(['mapperjs', 'helper', 'd3', 'utils', 'mdCache', 'lodash'], function(mappers, helper, d3, utils, mdCache, _) {

  "use strict";
  var perspective = {identifier: 'stc_dendrogram'};

  perspective.bind = function ($scope, $timeout, $container, source, parameters, isWidget, callback) {
    var self = this;
    ...
    try {
      ...
      var updateRecord = function(node) {
        if(node.id[0] !== '_') {
          var record = clean(node);
      ...
      var objectIdentifier = utils.getSourceObject(source).stc_identifier;
      var object = utils.getSourceObject(source);

      var fields = self.bindings;

      var dimensions = self.dimensions;

      if(!isWidget) {$scope.perspective.setExpandable(true);}
      var $content = $('<div>')
      ...
      var
          parentField     = self.bindings.stc_parent,
          iconField       = self.bindings.stc_icon,
          positionField   = self.bindings.stc_position,
          colorField      = self.bindings.stc_color,
          data,
      ...
      self.onRecordUpdate = function(record) {
        var id = getId(record);
        nodes.some(function(d) {
          if(getId(d) === id) {
            checkChanges(d, record); return true;
          }
        });
      };

      self.onRecordCreate = function(record) {
        var newNode = _(record).cloneDeep();
        if(nodeSelected) {
          addNode(newNode, nodeSelected);
          update(nodeSelected);
        } else {
      ...
      self.onRecordRemove = function(id) {
        removeNodeById(id, root);
      };

      var removeNodeById = function(id, parent) {
        var children = parent.children || parent._children || null;
        var child = _(children).find(function(child) {return child.id === id;});
        if(child) {
          children = _(children).reject(function(child) {return child.id === id;}).value();
          if(parent.children) {parent.children = children;}
          if(parent._children) {parent._children = children;}
          update(parent);
        } else {
          _(children).each(function(child) {removeNodeById(id, child);});
        }
      };


      var checkChanges = function(d, readOnly) {

        //Update the name
        if(getName(d) !== getName(readOnly)) {setName(d, getName(readOnly));}
        // update the parent node
        if(getParentId(d) !== getParentId(readOnly)) {
          var newParent, found = false;
          found = nodes.some(function(d2) {
            if(getId(d2) === getParentId(readOnly)) {
              newParent = d2; return true;
            }
          });
          if(found) {
            removeNode(d);
        ...
        //Update the color
        if(getColor(d) !== getColor(readOnly)) {setColor(d, getColor(readOnly));}
        if(getPosition(d) !== getPosition(readOnly)) {setPosition(d, getPosition(readOnly));}
        if(getIcon(d) !== getIcon(readOnly)) {
          setIcon(d, getIcon(readOnly));
        }
      ...
      var computeRoot = function(d, callback) {
        ...
        var conditions = {'field': parentField, 'operator': '=', 'value': id};

        mdCache.ready(function(){
          mappers.mapper(function(t, done) {
            helper.fetchRecords(t, source, {conditions: conditions}, function(err, children) {
              ...
              centerNodeName = object.stc_plural_name;
      ...
      // Quick fetch to decide a visual atribute (blue circle)
      var computeHasChildren = function(d, callback) {
        try {
          if (typeof d.hasChildren === 'undefined') {
            ...
            mappers.mapper(function(t, done) {
              var options = {
                onlyFirst: true,
                conditions: {'field': parentField, 'operator': '=', 'value': id}
              };
              helper.fetchRecords(t, source, options, function(err, child) {
                var has = !_(child).isEmpty();
                d.hasChildren = has;
                if(has) {d.childrenShowed = false;}
           ...
            });
          } else if(callback) {callback(d.hasChildren);}
      ...
      var onCircleClick = function(d, callback) {
        try {
          if(canClick) {
            ...
            update(d, function() {
              canClick = true;
              if(!isWidget) {self.openRecord(getId(d));}
      ...
      var update = function(source, callback) {
        try {
          ...
          //   _.each(root.children[0].children, function(child, i){
          //     if(typeof child.stc_name === 'undefined') {
          //       console.log('new item in pos', i ,child, child.y);
          //     }
          //   });
          // }
              ...
              .on("click", function(d, i, onClickCallback) {
                if(canClick) {
                  ...
                  if(d.hasChildren) {
                    ...
                    if(!isWidget) {self.openRecord(id);}
                  }
                }
              })
      ...
      //     $content.width(g[0].getBBox().width+left);
      //     $content.height($container.outerHeight()-15);
      //     clearTimeout(doit);
      ...
      var onNodeCentralClicked = function() {
        ...
        if(!isNodeCentralClicked) {
          ...
          moveSvg(function() {
            //STCFIX obsolete
            // if(!isWidget) {
            //   (centerNode)? self.openRecord(getId(centerNode)) : stoic.sidebar.openObjectEditor();
            // }
          });
        }
      ...
      perspective.root = root;
      perspective.showSpinner = showSpinner;
      perspective.hideSpinner = hideSpinner;
      perspective.update = update;

      perspective.expandAllRec = function(node, callback, depth) {
        if(node.hasChildren && depth <= maxX) {
          computeChildren(node, function() {
            var children = (node.childrenShowed)? node.children : node._children;
            var l = children.length;
            var n = 0;
            if(!node.childrenShowed) {toggle(node);}
            node.children.forEach(function(child) {
              computeHasChildren(child, function(has) {
                if(has) {
                  perspective.expandAllRec(child, function() {
                    if(++n === l) {callback();}
                  }, depth+1);
                } else {if(++n === l) {callback();} }
              });
            });
          });
        } else {callback();}
      ...
      perspective.collapseAllRec = function(node, callback) {
        var children, l, n;
        if(node === root && node.hasChildren) {
          children = (node.childrenShowed)? node.children : node._children;
          l = children.length;
          n = 0;
          if(!node.childrenShowed) {toggle(node);}
          node.children.forEach(function(child) {
            perspective.collapseAllRec(child, function() {if(++n === l) {callback();}});
          });
        } else {
          if(node.hasChildren) {
            children = (node.childrenShowed)? node.children : node._children;
      ...
      var rootField;
      // var rootField = (dimensions.special && dimensions.special)? dimensions.special.stc_root : null;

      var root;
      var start = function(id, name) {
        root = {'id':id, children:[], childrenShowed:true, hasChildren:true, x0:_h/2, y0:0};
      ...
      if(rootField) {
        mappers.mapper(function(t, done) {
          var options = {
            onlyFirst: true,
            conditions: ['id', '=', rootField]
          };
          helper.fetchRecords(t, source, options, function(err, record) {
            start(record.id, record.stc_name);
            done(err);
          });
        }, callback);
      } else {
